...
while ((match = classRegex.exec(content)) !== null) {
    ...
    // Some elements might have className flat cards but no style
    if (!hasShadow && (cls.includes('bg-[var(--card)]') || cls.includes('bg-card') || cls.includes('bg-white')) && cls.includes('border') && cls.includes('rounded')) {
...
while ((match = styleRegex.exec(content)) !== null) {
    ...
    if (hasShadow || isFlatCandidate) {
        // Find if there's a corresponding className on the same element to avoid duplicates/give full context
        results.push({
            type: 'style',
            lineNum: lineNum,
            content: match[0],
...
fs.writeFileSync('dashboard_exhaustive_v2.txt', out);
