...
(async () => {
  try {
    ...
    if (rowClicked) {
      ...
      console.log("Taking trade tracking detail page screenshot v2...");
      await page.screenshot({ path: 'trade-tracking-mobile-v2.png', fullPage: true });
    } else {
      console.log("Could not find any row in trading logs table to click.");
    }

    await browser.close();
    console.log("Tracking screenshot v2 captured successfully!");
  } catch (error) {
